...
func TestUpsertOffloadedFile(t *testing.T) {
	...
	err = db.UpsertOffloadedFile("/media/video.mp4", "abc123", 1, 1024000, time.Now())
	if err != nil {
		t.Fatalf("unexpected error on upsert: %v", err)
	}

	// Upsert again with different values (idempotent).
	err = db.UpsertOffloadedFile("/media/video.mp4", "abc456", 0, 2048000, time.Now())
	if err != nil {
...
func TestUpsertInvalidAutograph(t *testing.T) {
	...
	err = db.UpsertOffloadedFile("/media/video.mp4", "abc123", 2, 1024000, time.Now())
	if err == nil {
