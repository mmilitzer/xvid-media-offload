...
func runShrink(args []string) int {
	...
	if *dryRun {
		printShrinkDryRunReport(res, *verbose)
	} else {
		printShrinkApplyReport(res, *verbose)
	}
	return 0
}

func printShrinkDryRunReport(res *shrink.Result, verbose bool) {
...
func printShrinkApplyReport(res *shrink.Result, verbose bool) {
	...
	fmt.Printf("already offloaded:          %d\n", res.SkippedOffloaded)
	fmt.Printf("already sparse:             %d\n", res.SkippedSparse)
	fmt.Printf("skipped too young:          %d\n", res.SkippedTooYoung)
	fmt.Printf("skipped too small:          %d\n", res.SkippedTooSmall)
	fmt.Printf("skipped no autograph:       %d\n", res.SkippedNoAutograph)
	fmt.Printf("skipped file not found:     %d\n", res.SkippedMissingRemote)
